"""
File: load_big_sky_player_advanced.py
Last Modified: 2026-02-27
Purpose: Load current-season advanced season-level Big Sky player metrics from Sports Reference,
supplementing with Per 100 Possession data and the existing Big Sky player stats file.

Inputs:
- data/raw/sports_reference/big_sky_rosters.csv
- data/raw/sports_reference/big_sky_player_stats.csv
- Sports Reference Big Sky team season pages
...
- Source URL Pattern: https://www.sports-reference.com/cbb/schools/{team_slug}/men/{season}.html
...
ROSTER_INPUT_PATH = Path("data/raw/sports_reference/big_sky_rosters.csv")
PLAYER_STATS_INPUT_PATH = Path("data/raw/sports_reference/big_sky_player_stats.csv")
OUTPUT_PATH = Path("data/raw/sports_reference/big_sky_player_advanced_stats.csv")
...
def load_roster(path: Path) -> pd.DataFrame:
    ...
    roster_df["team_name"] = roster_df["team_name"].map(normalize_team_name)
    ...
    roster_df["team_name_key"] = roster_df["team_name"].map(normalize_team_key)
    ...
    roster_df = roster_df[
        ...
        & (roster_df["team_name_key"] != "")
    ].copy()

    roster_df = roster_df.drop_duplicates(
        subset=["team_name_key", "player_name_key"],
        keep="first",
...
def load_player_stats_if_available(path: Path) -> pd.DataFrame:
    ...
    if "player_name" not in stats_df.columns or "team_name" not in stats_df.columns:
        return pd.DataFrame()

    stats_df = stats_df.copy()
    stats_df["player_name"] = stats_df["player_name"].astype(str).str.strip()
    stats_df["team_name"] = stats_df["team_name"].map(normalize_team_name)
    stats_df["player_name_key"] = stats_df["player_name"].map(normalize_name)
    stats_df["team_name_key"] = stats_df["team_name"].map(normalize_team_key)
...
def team_page_url(team_name: str, season: int) -> str:
    team_key = normalize_team_key(team_name)
    team_slug = TEAM_SLUG_MAP.get(team_key)

    if not team_slug:
        raise ValueError(f"No Sports Reference team slug mapping found for team: {team_name}")
...
def select_row_by_name(df: pd.DataFrame, team_name_key: str, player_name_key: str) -> Optional[pd.Series]:
    if df.empty or "player_name_key" not in df.columns:
        return None

    matches = df[df["player_name_key"] == player_name_key].copy()

    if "team_name_key" in matches.columns:
        matches = matches[matches["team_name_key"] == team_name_key].copy()
...
def build_player_record(
    ...
    team_name = normalize_team_name(roster_row["team_name"])
...
def deduplicate_players(df: pd.DataFrame) -> pd.DataFrame:
    ...
    output_df["team_name_key"] = output_df["team_name"].map(normalize_team_key)
    output_df["non_null_count"] = output_df.notna().sum(axis=1)

    output_df = (
        output_df.sort_values(
            by=["team_name_key", "player_name_key", "season", "non_null_count"],
            ascending=[True, True, True, False],
        )
        .drop_duplicates(subset=["team_name_key", "player_name_key", "season"], keep="first")
        .drop(columns=["team_name_key", "player_name_key", "non_null_count"])
        .reset_index(drop=True)
    )
...
def main() -> None:
    ...
    team_tables: dict[str, dict[str, object]] = {}

    unique_teams = (
        roster_df[["team_name", "team_name_key"]]
        .drop_duplicates()
    ...
    for _, team_row in unique_teams.iterrows():
        team_name = normalize_team_name(team_row["team_name"])
        team_name_key = team_row["team_name_key"]

        try:
            source_team_url = team_page_url(team_name, season)
            html = fetch_html(session, source_team_url)
            ...
            team_tables[team_name_key] = {
                "advanced_df": advanced_df,
                "per100_df": per100_df,
                "source_team_url": source_team_url,
            }
        except Exception as exc:
            print(f"Failed to load advanced tables for {team_name}: {exc}")
            team_tables[team_name_key] = {
    ...
    for _, roster_row in roster_df.iterrows():
        team_name_key = roster_row["team_name_key"]
        player_name_key = roster_row["player_name_key"]

        team_data = team_tables.get(
            team_name_key,
            {"advanced_df": pd.DataFrame(), "per100_df": pd.DataFrame(), "source_team_url": None},
        )

        player_stats_row = select_row_by_name(player_stats_df, team_name_key, player_name_key)
        advanced_row = select_row_by_name(team_data["advanced_df"], team_name_key, player_name_key)
        per100_row = select_row_by_name(team_data["per100_df"], team_name_key, player_name_key)
        ...
        record = build_player_record(
            ...
            season=season,
            source_team_url=normalize_text(team_data["source_team_url"]),
        )
        records.append(record)

    if not records:
        raise ValueError("No Big Sky player advanced records were created.")
    ...
    print(f"Big Sky player advanced stats saved to: {output_path}")
