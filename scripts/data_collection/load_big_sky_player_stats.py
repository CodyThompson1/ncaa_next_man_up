"""
File: load_big_sky_player_stats.py
Purpose: Load current-season season-level player stats for Big Sky players
         outside of Montana from Sports Reference.

Inputs:
- data/raw/sports_reference/big_sky_rosters.csv
- Sports Reference player pages from roster links
- Sports Reference team season pages for Big Sky schools
...
OUTPUT_PATH = Path("data/raw/sports_reference/big_sky_player_season_stats.csv")

EXCLUDED_TEAMS = {"Montana"}
...
def normalize_team_name(value: object) -> str:
    ...
    team_name = TEAM_NAME_REPLACEMENTS.get(team_name, team_name)
    return team_name
...
def get_team_slug(team_name: str) -> str:
    ...
    if normalized_team_name not in TEAM_SLUG_MAP:
        raise ValueError(f"No team slug mapping found for team: {normalized_team_name}")

    return TEAM_SLUG_MAP[normalized_team_name]


def build_team_page_url(team_name: str, season: int) -> str:
...
def load_roster(path: Path) -> pd.DataFrame:
    ...
    roster_df = roster_df[
        ...
        & (~roster_df["team_name"].isin(EXCLUDED_TEAMS))
    ].copy()

    roster_df = roster_df.drop_duplicates(
        subset=["player_name_key", "team_name", "season"],
        keep="first",
    ).reset_index(drop=True)

    if roster_df.empty:
        raise ValueError("Roster file contains no valid non-Montana Big Sky player rows after cleaning.")
...
def get_current_season_from_roster(roster_df: pd.DataFrame) -> int:
    ...
    if not season_values:
        raise ValueError("Could not determine current season from Big Sky roster file.")
...
def prepare_candidate_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    working_df["team_normalized"] = working_df["team"].astype(str).map(normalize_team_name).str.lower()
...
def select_best_player_season_row(
    ...
    target_team_name = normalize_team_name(team_name).lower()
    ...
    for table in tables:
        ...
        season_df = working_df[
            ...
            & (working_df["team_normalized"] == target_team_name)
        ].copy()
...
def get_class_value(
    ...
    per_game_row: Optional[pd.Series],
    totals_row: Optional[pd.Series],
    team_row: Optional[pd.Series],
) -> Optional[str]:
    for source in [per_game_row, totals_row, team_row, roster_row]:
        if source is None:
            continue
        if "class" in source.index:
            value = normalize_text(source.get("class"))
...
def deduplicate_players(df: pd.DataFrame) -> pd.DataFrame:
    ...
    deduped_df = (
        deduped_df.sort_values(
            by=["team_name", "player_name_key", "season", "non_null_count"],
            ascending=[True, True, True, False],
        )
        .drop_duplicates(subset=["team_name", "player_name_key", "season"], keep="first")
...
def main() -> None:
    ...
    current_season = get_current_season_from_roster(roster_df)
    roster_df = roster_df[roster_df["season"] == current_season].copy()

    if roster_df.empty:
        raise ValueError("No Big Sky non-Montana roster rows found for the current season.")

    session = create_session()

    team_player_tables: dict[str, pd.DataFrame] = {}
    for team_name in sorted(roster_df["team_name"].dropna().unique().tolist()):
        team_url = build_team_page_url(team_name, current_season)
        team_html = fetch_html(session, team_url)
        ...
        if team_player_df.empty:
            raise ValueError(f"Could not locate a usable team player stats table for {team_name}.")

        team_player_tables[team_name] = team_player_df

    records = []
    for _, roster_row in roster_df.iterrows():
        team_name = normalize_team_name(roster_row["team_name"])
        team_player_df = team_player_tables[team_name]
    ...
    if not records:
        raise ValueError("No Big Sky player records were created.")
    ...
    print(f"Big Sky player season stats saved to: {output_path}")
