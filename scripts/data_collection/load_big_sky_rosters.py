"""
File: load_big_sky_rosters.py
Last Modified: 2026-02-18
Purpose: Load the current-season rosters for all non-Montana Big Sky men's basketball teams from Sports Reference.

Inputs:
- Sports Reference team roster pages for current-season non-Montana Big Sky teams
...
TEAM_URL_PATTERN = "https://www.sports-reference.com/cbb/schools/{team_slug}/men/{season}.html"
OUTPUT_PATH = Path("data/raw/sports_reference/big_sky_rosters.csv")
REQUEST_TIMEOUT = 30
...
BIG_SKY_TEAMS = [
    {"team_name": "Eastern Washington", "team_slug": "eastern-washington"},
    {"team_name": "Idaho", "team_slug": "idaho"},
    {"team_name": "Idaho State", "team_slug": "idaho-state"},
    {"team_name": "Montana State", "team_slug": "montana-state"},
    {"team_name": "Northern Arizona", "team_slug": "northern-arizona"},
    {"team_name": "Northern Colorado", "team_slug": "northern-colorado"},
    {"team_name": "Portland State", "team_slug": "portland-state"},
    {"team_name": "Sacramento State", "team_slug": "sacramento-state"},
    {"team_name": "Weber State", "team_slug": "weber-state"},
]
...
def build_team_url(team_slug: str, season: int) -> str:
    return TEAM_URL_PATTERN.format(team_slug=team_slug, season=season)
...
def find_roster_table(soup: BeautifulSoup) -> BeautifulSoup:
    ...
    raise ValueError("Roster table not found on the Sports Reference team page.")
...
def parse_roster_table(
    ...
    return map_output_fields(raw_df, season, team_name)


def clean_weight_value(value: object) -> Optional[str]:
    normalized = normalize_text(value)
    if normalized is None:
        return None

    cleaned = normalized.replace("lb", "").replace("lbs", "").strip()
    return cleaned or None
...
def standardize_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    standardized_df = standardized_df.drop_duplicates(
        subset=["team_name", "player_name", "season"],
        keep="first",
    ).reset_index(drop=True)

    return standardized_df[ordered_columns]
...
def validate_output(df: pd.DataFrame) -> None:
    ...
    if df["conference_name"].isna().any():
        raise ValueError("conference_name contains null values.")
...
def load_team_roster(
    ...
    roster_df = parse_roster_table(roster_table, season, team_name)
    roster_df = standardize_fields(roster_df)
    return roster_df


def load_big_sky_rosters() -> pd.DataFrame:
    ...
    for team in BIG_SKY_TEAMS:
        ...
        try:
            team_roster_df = load_team_roster(
                session=session,
                season=season,
                team_name=team_name,
                team_slug=team_slug,
            )
            all_rosters.append(team_roster_df)
        except Exception as exc:
            print(f"Failed to load roster for {team_name}: {exc}", file=sys.stderr)

    if not all_rosters:
        raise ValueError("No Big Sky roster records were collected from Sports Reference.")

    combined_df = pd.concat(all_rosters, ignore_index=True)
    combined_df = standardize_fields(combined_df)
...
def main() -> None:
    ...
    - Source URL Pattern: https://www.sports-reference.com/cbb/schools/<school>/men/{season}.html
    ...
    """
    try:
        rosters_df = load_big_sky_rosters()
        print(f"Big Sky roster rows saved: {len(rosters_df)}")
        print(f"Output written to: {OUTPUT_PATH.resolve()}")
    except Exception as exc:
        print(f"Failed to load Big Sky rosters: {exc}", file=sys.stderr)
        raise
