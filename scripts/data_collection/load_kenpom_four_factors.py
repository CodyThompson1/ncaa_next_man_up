...
Purpose: Load current-season KenPom four-factors endpoint data for all D1 teams and export all D1, Montana-only, and Big Sky-only raw outputs.

Inputs:
- KenPom API four-factors endpoint
- KenPom API ratings endpoint
- scripts/utilities/kenpom_api_utils.py

Outputs:
- data/raw/kenpom/four_factors/all_d1_four_factors.csv
- data/raw/kenpom/four_factors/montana_four_factors.csv
- data/raw/kenpom/four_factors/big_sky_four_factors.csv
"""
...
ENDPOINT_FOLDER_NAME = "four_factors"
...
def build_output_paths() -> dict[str, Path]:
    ...
    return {
        "all_d1": base_dir / "all_d1_four_factors.csv",
        "montana": base_dir / "montana_four_factors.csv",
        "big_sky": base_dir / "big_sky_four_factors.csv",
    }


def load_conference_lookup(season: int) -> pd.DataFrame:
    ratings_df = fetch_endpoint_dataframe(endpoint=CONFERENCE_BACKFILL_ENDPOINT, season=season)
    ...
    validate_required_columns(ratings_df, REQUIRED_COLUMNS, "KenPom ratings conference lookup")
    validate_required_values(ratings_df, REQUIRED_COLUMNS, "KenPom ratings conference lookup")
...
def backfill_conference_name(
    ...
    output_df = output_df.merge(
        conference_lookup_df.rename(columns={"conference_name": "conference_name_lookup"}),
        on=["team_name", "season"],
        how="left",
    )
    ...
    if "conference_name" not in output_df.columns:
        ...
        output_df["conference_name"] = output_df["conference_name"].replace(
            "",
            pd.NA,
        ).fillna(output_df["conference_name_lookup"])
...
def load_all_d1_four_factors(season: int) -> pd.DataFrame:
    ...
    validate_non_empty_dataframe(df, "KenPom all D1 four-factors")
    validate_required_columns(df, REQUIRED_COLUMNS, "KenPom all D1 four-factors")
    validate_required_values(df, REQUIRED_COLUMNS, "KenPom all D1 four-factors")
...
def build_subset_outputs(all_d1_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    validate_non_empty_dataframe(montana_df, "KenPom Montana four-factors")
    validate_required_columns(montana_df, REQUIRED_COLUMNS, "KenPom Montana four-factors")
    validate_required_values(montana_df, REQUIRED_COLUMNS, "KenPom Montana four-factors")

    validate_non_empty_dataframe(big_sky_df, "KenPom Big Sky four-factors")
    validate_required_columns(big_sky_df, REQUIRED_COLUMNS, "KenPom Big Sky four-factors")
    validate_required_values(big_sky_df, REQUIRED_COLUMNS, "KenPom Big Sky four-factors")
...
def main() -> None:
    ...
    all_d1_df = load_all_d1_four_factors(season=season)
    ...
    print_export_summary(
        dataset_name="KenPom four-factors",
        all_d1_df=all_d1_df,
        montana_df=montana_df,
        big_sky_df=big_sky_df,
        output_paths=output_paths,
