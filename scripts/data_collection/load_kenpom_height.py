"""
File: load_kenpom_height.py
Last Modified: 2026-02-19
Purpose: Load current-season KenPom height endpoint data for all D1 teams and export all D1, Montana-only, and Big Sky-only raw outputs.

Inputs:
- KenPom API height endpoint
- KenPom API ratings endpoint
- scripts/utilities/kenpom_api_utils.py

Outputs:
- data/raw/kenpom/height/all_d1_height.csv
- data/raw/kenpom/height/montana_height.csv
- data/raw/kenpom/height/big_sky_height.csv
...
def build_output_paths() -> dict[str, Path]:
    ...
    return {
        "all_d1": base_dir / "all_d1_height.csv",
        "montana": base_dir / "montana_height.csv",
        "big_sky": base_dir / "big_sky_height.csv",
    }
...
def load_all_d1_height(season: int) -> pd.DataFrame:
    ...
    validate_non_empty_dataframe(df, "KenPom all D1 height")
    validate_required_columns(df, REQUIRED_COLUMNS, "KenPom all D1 height")
    validate_required_values(df, REQUIRED_COLUMNS, "KenPom all D1 height")
...
def build_subset_outputs(all_d1_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    validate_non_empty_dataframe(montana_df, "KenPom Montana height")
    validate_required_columns(montana_df, REQUIRED_COLUMNS, "KenPom Montana height")
    validate_required_values(montana_df, REQUIRED_COLUMNS, "KenPom Montana height")

    validate_non_empty_dataframe(big_sky_df, "KenPom Big Sky height")
    validate_required_columns(big_sky_df, REQUIRED_COLUMNS, "KenPom Big Sky height")
    validate_required_values(big_sky_df, REQUIRED_COLUMNS, "KenPom Big Sky height")
...
def main() -> None:
    ...
    all_d1_df = load_all_d1_height(season=season)
