"""
File: load_kenpom_misc_stats.py
Last Modified: 2026-02-18
Purpose: Load current-season KenPom misc-stats endpoint data for all D1 teams and export all D1, Montana-only, and Big Sky-only raw outputs.

Inputs:
- KenPom API misc-stats endpoint
- KenPom API ratings endpoint
- scripts/utilities/kenpom_api_utils.py

Outputs:
- data/raw/kenpom/misc_stats/all_d1_misc_stats.csv
- data/raw/kenpom/misc_stats/montana_misc_stats.csv
- data/raw/kenpom/misc_stats/big_sky_misc_stats.csv
...
ENDPOINT_FOLDER_NAME = "misc_stats"
...
def build_output_paths() -> dict[str, Path]:
    ...
    return {
        "all_d1": base_dir / "all_d1_misc_stats.csv",
        "montana": base_dir / "montana_misc_stats.csv",
        "big_sky": base_dir / "big_sky_misc_stats.csv",
    }
...
def load_all_d1_misc_stats(season: int) -> pd.DataFrame:
    ...
    validate_non_empty_dataframe(df, "KenPom all D1 misc stats")
    validate_required_columns(df, REQUIRED_COLUMNS, "KenPom all D1 misc stats")
    validate_required_values(df, REQUIRED_COLUMNS, "KenPom all D1 misc stats")
...
def build_subset_outputs(all_d1_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    validate_non_empty_dataframe(montana_df, "KenPom Montana misc stats")
    validate_required_columns(montana_df, REQUIRED_COLUMNS, "KenPom Montana misc stats")
    validate_required_values(montana_df, REQUIRED_COLUMNS, "KenPom Montana misc stats")

    validate_non_empty_dataframe(big_sky_df, "KenPom Big Sky misc stats")
    validate_required_columns(big_sky_df, REQUIRED_COLUMNS, "KenPom Big Sky misc stats")
    validate_required_values(big_sky_df, REQUIRED_COLUMNS, "KenPom Big Sky misc stats")
...
def main() -> None:
    ...
    all_d1_df = load_all_d1_misc_stats(season=season)
    ...
    print_export_summary(
        dataset_name="KenPom misc stats",
        all_d1_df=all_d1_df,
        montana_df=montana_df,
        big_sky_df=big_sky_df,
        output_paths=output_paths,
